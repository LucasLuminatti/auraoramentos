...
/** RULE-106 — peças avulsas do sistema de perfil. Uma linha por peça, no mesmo padrão
 *  dos componentes; sem elas o subtotal do sistema teria valor sem origem visível. */
function rowsAcessoriosSistema(sis: SistemaIluminacao, atributosMap: AtributosMap): string {
  return (sis.acessorios ?? [])
    ...
    <tr class="item-row">
      ...
      <td class="desc-cell">
        <div class="desc-name"><span class="comp-tag">Acessório</span> ${esc(descRica)}</div>
      </td>
...
function rowComponente(c: ItemComposicao, atributosMap: AtributosMap): string {
  ...
  return `
    <tr class="item-row comp-sub-row">
      ...
      <td class="qty-cell">${n(c.quantidade)} un</td>
      <td class="watts-cell">—</td>
...
export function blocoComposto(item: ItemLuminaria, indexComposto: number, atributosMap: AtributosMap): string {
  ...
  const ancoraCampoTecnico = item.potencia_watts
    ? `${n(item.potencia_watts)}W${item.tensao ? ` / ${n(item.tensao)}V` : ""}`
    : "—";
