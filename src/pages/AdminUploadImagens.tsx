...
import { ArrowLeft, Upload, ImageIcon, CheckCircle, AlertTriangle, XCircle, ChevronDown, Download, ShieldCheck } from "lucide-react";
...
const AdminUploadImagens = () => {
  ...
  // Fetch all product codes with pagination (>1000 rows)
        ...
        <Card className="border-primary/20 bg-primary/5">
          ...
          <CardContent>
            <ul className="space-y-1.5 text-sm text-muted-foreground">
              ...
              <li>• <strong>Nome do arquivo</strong> deve ser o código do produto (ex: <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-foreground">LM2439.jpg</code>)</li>
              <li>• <strong>Quantidade:</strong> ilimitada — o sistema processa em lotes sem travar</li>
              <li>• Arquivos sem correspondência na base serão listados para revisão</li>
            </ul>
          </CardContent>
        </Card>
        ...
        {(phase === "idle" || phase === "ready") && (
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">JPG, PNG, WEBP • Máx. 2MB cada • Sem limite de quantidade</p>
            </div>
            <input
              ref={inputRef}
              type="file"
        ...
        {(phase === "ready" || phase === "uploading" || phase === "done") && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {validFiles.length + noMatchFiles.length + invalidFiles.length} arquivos analisados
              </CardDescription>
            </CardHeader>
                ...
                <Collapsible open={showNoMatch} onOpenChange={setShowNoMatch}>
                  <CollapsibleTrigger asChild>
                    <button className="flex w-full items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3 hover:bg-destructive/15 transition-colors">
                      ...
                      <span className="text-sm font-medium text-destructive flex-1 text-left">
                        {noMatchFiles.length} sem correspondência na base
                      </span>
                  ...
                  <CollapsibleContent>
                    ...
                    <Button
                      ...
                      onClick={() => exportCsv(noMatchFiles, "sem-correspondencia.csv")}
                    >
                  ...
                  <CollapsibleTrigger asChild>
                    <button className="flex w-full items-center gap-3 rounded-lg border border-border bg-muted/50 p-3 hover:bg-muted transition-colors">
                      ...
                      <span className="text-sm font-medium text-muted-foreground flex-1 text-left">
                        {invalidFiles.length} inválidos (formato ou tamanho)
                      </span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                {phase === "uploading" ? "Enviando..." : "Upload Concluído"}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <CollapsibleTrigger asChild>
                    <button className="flex w-full items-center gap-3 rounded-lg border border-green-500/30 bg-green-500/10 p-3 hover:bg-green-500/15 transition-colors">
                      ...
                      <span className="text-sm font-medium text-green-700 flex-1 text-left">
                        {successList.length} enviados com sucesso
                      </span>
                ...
                <Collapsible open={showErrors} onOpenChange={setShowErrors}>
                  <CollapsibleTrigger asChild>
                    <button className="flex w-full items-center gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3 hover:bg-destructive/15 transition-colors">
                      ...
                      <span className="text-sm font-medium text-destructive flex-1 text-left">
                        {uploadErrors.length} erros de upload
                      </span>
