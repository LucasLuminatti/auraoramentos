...
- [x] **Phase 5: PDF Redesign** — Reconstrução do PDF com layout tipográfico limpo, remoção das 4 caixas e texto final formatado
- [x] **Phase 6: Filtros & Smoke** — Filtros por arquiteto (clientes/produtos/pedidos), filtros combináveis e smoke test manual em prod
...
| 5. PDF Redesign | 5/5 | Complete | 2026-05-04 |
| 6. Filtros & Smoke | 5/5 | Complete | 2026-05-07 |
...
## Milestone Closure — v1.0

**Shipped:** 2026-05-07
**Duration:** 15 days (2026-04-23 → 2026-05-07)
**Total commits:** 163
**Total plans:** 28
**Diff:** 259 arquivos · +35.447 / −3.610 LOC
**Migrations:** 9 aplicadas (todas aditivas, zero destrutivas)
**Edge functions deployed:** create-colaborador, import-produtos, request-access, review-access
**Smoke prod:** 8/8 itens passed (2026-05-07, validação cross-phase em produção)

### Top 6 accomplishments

1. **Arquiteto como entidade própria** com FK em `clientes` + `produtos` — filtros confiáveis, CRUD admin completo
2. **Importação CSV de produtos** + ImportMaster XLSX one-shot (2.088 variants oficiais) — fim das planilhas paralelas
3. **Drive RLS por `user_id`** + bucket privado + signed URLs — isolamento entre colaboradores, admin vê tudo (validado em prod com 2 contas reais)
4. **Admin reorganizado** em 5 sub-tabs (Início/Cadastros/Pedidos/Preços/Exceções) com URL state
5. **PDF redesign v2** (Playfair + Inter, sem 4 caixas, prose final) com **roteador v1/v2** retro-compatível para snapshots antigos
6. **Filtros por arquiteto** em Clientes/Produtos/Pedidos + combinações em Pedidos (cliente + período + status) com URL state e popover mobile

### Requirements outcome

- **Entregues:** 40/42
- **Obsoletos:** 1 (PROD-02 — DB tinha 0 produtos sem desc/preço; substituído por SKUs coringa AU001..AU016)
- **Deferidos para v1.1+:** 1 (IMP-02 — preço por CSV; preço atualiza 1x/mês em prod, vira phase de preços)
...
*Phase 5 planned + completed: 2026-05-04 (5 plans, 5 reqs entregues — PDF-01..05)*
*Phase 6 planned + completed: 2026-05-07 (5 plans, 5 reqs entregues — FIL-01..04 + WRAP-01)*
*Milestone v1.0 archived: 2026-05-07*
