# Requirements Archive: v1.2 Correções UAT + UX do Wizard de Sistemas de Iluminação
...
- [x] **CAT-01**: Colaborador encontra na busca de perfil/driver TODOS os produtos da família (ex.: PERFIL CANTONEIRA, WALL WASHER, LM3475, LM3291), corrigindo `tipo_produto` errado/nulo via migração SQL aditiva (WALL WASHER → `'perfil'`, pois `'wall_washer'` não é valor válido no CHECK).
- [x] **CAT-02**: O aviso/dica exibido ao adicionar o MAGNETO corresponde ao MAGNETO (não ao TINY MAGNETO) — corrigir o mapeamento/dado.

### SIST — Sistemas (advisory que cabe em v1.2)

- [x] **SIST-04**: Ao adicionar um item da linha Tiny (ex.: TINY SPOT 24V), o sistema avisa que requer driver 24V e oferece a opção de incluí-lo. _(advisory simples; a montagem completa de sistemas compostos é v1.3 — SIST-01/02/03.)_

### TENS — Tensão / Voltagem (redução de erro)

- [x] **TENS-01**: A voltagem do driver é inferida/validada a partir da fita; quando a voltagem não bate, o sistema avisa (em vez de seleção manual silenciosa).
- [x] **TENS-02**: Colaborador usa tensão diferente em ambientes diferentes sem bloqueio indevido (remover o link à tensão do ambiente anterior), com a agregação global de drivers agrupando por (código + voltagem).

### CALC — Cálculo / Contabilização

- [x] **CALC-01**: Fita sem perfil contabiliza a metragem informada (exigir/preencher a metragem manual; não deixar passar como 0m → R$ 0,00 silencioso).
- [x] **CALC-02**: A metragem/comprimento do perfil reflete automaticamente na descrição após inserir o código.
- [x] **CALC-03**: A sugestão de passadas de fita é editável (permite reduzir) e respeita a regra de que perfil de 50mm aceita até 3 passadas (sincronizar `passadas_padrao`).
...
- [x] **UX-02**: Ao escolher a fita, o sistema sugere automaticamente um driver compatível (voltagem + potência) como default. _(estende TENS-01 de "avisar" para "já preencher certo")_
...
| TENS-01 | Phase 15 | Complete |
| TENS-02 | Phase 15 | Complete |
| SIST-04 | Phase 15 | Complete |
| UX-02 | Phase 15 | Complete |
| CALC-01 | Phase 16 | Complete |
| CALC-02 | Phase 16 | Complete |
| CALC-03 | Phase 16 | Complete |
